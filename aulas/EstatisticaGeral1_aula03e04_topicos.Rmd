...
**Exemplo 1**: Variável em estudo ($X$) - PIB do Brasil, $X \in \mathbb{R}$.
<!-- , $X \sim Normal(\mu, \sigma^2)$. $E(X) = \mu$. -->

**Exemplo 2**: Variável em estudo ($X$) - avaliação do governo, positiva ou negativa, $X \in \{0, 1\}$.
...
```{r exemplo}
# x <- scan() # apos o comando scan(), copiar e colar os valores separados por espacos ou nova linha
...
Exercício 6 da Lista 1-2 e 1-3 do material `Probabilidade e Estatística EAD` em [ufrgs.br/probabilidade-estatistica](ufrgs.br/probabilidade-estatistica):
```{r exemplo2, fig.width=3.5, fig.height=2.5, fig.align='center',}
