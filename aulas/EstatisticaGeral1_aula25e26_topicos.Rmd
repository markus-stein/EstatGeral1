...
Definição **vetor aleatório discreto**: denotamos $(X, Y)$ um vetor aleatório onde $X$ e $Y$ são v.a. discretas definidas no mesmo espaço amostral $S$ de um experimento aleatório $E$, assumindo particulares pares de valores $(x,y)$.
...
Definição **Função massa de probabilidade conjunta**: Seja $(X, Y)$ um vetor aleatório, a f.m.p. conjunta $p(x, y)$ é definida para cada par $(x, y)$ por 
$$p(x, y) = P([X=x] \cap [Y=y]).$$   
...
Definição **Funções massa de probabilidade marginais**: Seja $(X, Y)$ um vetor aleatório, as f.m.p. marginais de $X$ e $Y$ são dadas respectivamente por 
$$p(x) = \sum_y p(x, y) \text{ e } p(y) = \sum_x p(x, y).$$
* $p(x)$ e $p(x)$ seguem as propriedades de uma f.m.p.

## 2. O caso de duas variáveis $X$ e $Y$ **contínuas**
